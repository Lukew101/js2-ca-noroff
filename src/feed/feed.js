const getPosts = async () => {
  try {
    const response = await fetch(
      "https://v2.api.noroff.dev/social/posts?limit=10",
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
          "X-Noroff-API-Key": "token value",
        },
      }
    );
    if (response.ok) {
...
const displayPosts = (posts) => {
  ...
  posts.forEach((post) => {
    ...
    feedPost.classList.add("card");
    feedPost.classList.add("w-100");
    feedPost.innerHTML = `
                <div class="card-body">
                  <div class="d-flex flex-column gap-2 mb-4 text-start">
                    <h2>${post.title}</h2>
                      <p class="m-0 post-posted-time">
                        ${timePostedAgo}
                      </p>
                  </div>
                  ...
                  <div class="d-flex small-grey-text justify-content-between">
                    ...
                    <p class="m-0 pointer">${post._count.comments} ${comments}</p>
                  </div>
                  <div
